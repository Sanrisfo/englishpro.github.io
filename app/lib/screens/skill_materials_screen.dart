  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (materialsRes['success'] == true) {
        ...
        _errorMessage = materialsRes['message'] ?? 'No se pudieron cargar los materiales';
      }
  ...
  Future<void> _openMaterial(MaterialModel m) async {
    ...
    if (t == 'pdf' && (m.archivoUrl?.isNotEmpty ?? false)) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => PDFViewerWidget(pdfUrl: m.archivoUrl!, title: m.titulo),
        ),
      );
      return;
    }
                        ...
                        ..._quizzes.map((q) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              title: Text(title),
                              subtitle: Text(minutos != null ? 'Tipo: $tipo â€¢ Tiempo: $minutos min' : 'Tipo: $tipo'),
                              onTap: () {
                                final user = context.read<AuthProvider>().user;
                                final userId = user?.idUsuario;
                                if (userId == null) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('Debes iniciar sesiÃ³n para realizar la actividad')),
                                  );
                                  return;
                                }
                                Navigator.push(
