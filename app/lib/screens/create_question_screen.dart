...
class _CreateQuestionScreenState extends State<CreateQuestionScreen> {
  ...
  // Multiple choice
  List<TextEditingController> _mcOptCtrls = [
  ...
  // Matching
  List<TextEditingController> _matchAnswersCtrls = [
    TextEditingController(),
    TextEditingController(),
    TextEditingController(),
  ];
  ...
  // Completion
  List<Map<String, TextEditingController>> _completionRows = List.generate(
    5,
    (_) => {
    ...
    try {
      if (widget.questionToEdit != null) {
        ...
        if (_type == 'multiple_choice') {
          ...
          if (opts != null) {
            _mcOptCtrls = opts
                .map((o) => TextEditingController(text: o['texto_opcion']))
                .toList();
            for (int i = 0; i < opts.length; i++) {
              if (opts[i]['es_correcta'] == true) _mcCorrect.add(i);
            }
            // Ensure at least 3 controllers
            while (_mcOptCtrls.length < 3)
              _mcOptCtrls.add(TextEditingController());
          }
        } else if (_type == 'completion') {
          final sentences = q['completion_sentences'] as List?;
          if (sentences != null) {
            _completionRows = [];
            for (final sentenceData in sentences) {
              final template = sentenceData['texto_template'] as String? ?? '';
              final gaps = sentenceData['gaps'] as List?;
              final answer = (gaps != null && gaps.isNotEmpty)
                  ? (gaps.first['correct_text'] as String? ?? '')
                  : '';

              // Reconstruct the full sentence for the UI
              final fullSentence = template.contains('{{1}}')
                  ? template.replaceFirst('{{1}}', answer)
                  : template;

              _completionRows.add({
                'sentence': TextEditingController(text: fullSentence),
                'correct': TextEditingController(text: answer),
              });
            }
            // Ensure at least 5 rows for the UI
            while (_completionRows.length < 5) {
              _completionRows.add({
                'sentence': TextEditingController(),
                'correct': TextEditingController(),
              });
        ...
        // NOTE: For Matching, we rely on the teacher re-entering data if they want to edit, 
  ...
  void dispose() {
    ...
    for (final r in _matchStatements)
      (r['text'] as TextEditingController).dispose();
  ...
  Future<void> _submit() async {
    ...
    if (texto.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Texto requerido')));
      return;
    }
    ...
    try {
      ...
      if (type == 'multiple_choice') {
        ...
        for (int i = 0; i < _mcOptCtrls.length; i++) {
          ...
          opts.add({
            'id_pregunta': currentQId,
            'texto_opcion': t,
            'es_correcta': _mcCorrect.contains(i),
            'orden': i + 1,
          });
        }
        ...
        if (isEdit) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Updated basic info. Editing matching pairs is not fully supported yet.',
              ),
            ),
          );
        } else {
          // ... (Existing Creation Logic)
          final answers = _matchAnswersCtrls
              .map((c) => c.text.trim())
          ...
          final statements = _matchStatements
              ...
              .map(
                (r) => {
                  'texto': (r['text'] as TextEditingController).text.trim(),
                  'answer_index': (r['answer'] as int),
                },
              )
              .toList();
          if (statements.isEmpty || statements.length > 5)
            throw 'Matching: 1..5 enunciados';
          if (answers.length < statements.length || answers.length > 7)
            throw 'Matching: respuestas B entre A..7';
        ...
        if (isEdit) {
          ...
          if (sentences.isEmpty) throw 'Completion: al menos una oración';

          await supabase.rpc(
            'update_completion_question',
            params: {'p_id_pregunta': qId, 'p_sentences': sentences},
          );
        } else {
          // ... (Existing Creation Logic)
          if (_completionRows.length < 5 || _completionRows.length > 6)
            throw 'Completion: 5..6 oraciones';
          final sentences = <Map<String, String>>[];
          for (final r in _completionRows) {
            final sentence = r['sentence']!.text.trim();
            final correct = r['correct']!.text.trim();
  ...
  Widget _buildTypeSelector() {
    ...
    final availableTypes = typesData
        .where((t) => _allowedTypes.contains(t['id']))
        .toList();
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: isSelected
                          ...
                          : Colors.grey[100], // Azul si seleccionado
                    ...
                    child: Icon(
                      ...
                      color: isSelected
                          ? Colors.white
                          : Colors.grey[400], // Blanco si seleccionado
                      size: 28,
                    ),
                  ),
                ],
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    RichTextField(
                      ...
                      maxLines: 3,
                      decoration: _inputDeco('Texto de la pregunta'),
                    ),
                    const SizedBox(height: 16),
                    const SizedBox(height: 16),
                    RichTextField(
                      controller: _explanationCtrl,
                      label: 'Explicación general (opcional)',
                      maxLines: 2,
                      decoration: _inputDeco('Explicación general (opcional)'),
                    ),
                    const SizedBox(height: 16),
                    Row(
                      children: [
                    ...
                    if (_type == 'multiple_choice') ...[
                      Row(
                        ...
                        children: [
                          ...
                          TextButton.icon(
                            onPressed: _mcOptCtrls.length < 5
                                ? () => setState(
                                    () => _mcOptCtrls.add(
                                      TextEditingController(),
                                    ),
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: RichTextField(
                                  controller: e.value,
                                  label: 'Opción ${e.key + 1}',
                                  decoration: _inputDeco('Opción ${e.key + 1}'),
                                ),
                              ),
                    ...
                    // matching
                      ...
                      const SizedBox(height: 8),
                      ..._matchAnswersCtrls.asMap().entries.map(
                        (e) => Padding(
                            ...
                            children: [
                              ...
                              if (_matchAnswersCtrls.length > 1)
                                IconButton(
                                  icon: const Icon(Icons.close),
                                  onPressed: () => setState(
                                    () => _matchAnswersCtrls.removeAt(e.key),
                                  ),
                                ),
                            ],
                          ),
                      ...
                      Align(
                        ...
                        child: TextButton.icon(
                          onPressed: _matchAnswersCtrls.length < 7
                              ? () => setState(
                                  () => _matchAnswersCtrls.add(
                                    TextEditingController(),
                                  ),
                                )
                              : null,
                          ...
                          child: Row(
                            children: [
                              Expanded(
                                ...
                                child: RichTextField(
                                  controller: row['text'],
                                  label: 'Enunciado A${i + 1}',
                                  decoration: _inputDeco('Enunciado A${i + 1}'),
                                ),
                              ),
                    ...
                    // completion
                      ...
                      const Text(
                        'Escribe la oración completa y la palabra que será el gap.',
                          ...
                          child: Column(
                            children: [
                              RichTextField(
                                ...
                                label: 'Oración Completa ${e.key + 1}',
                                decoration: _inputDeco(
                                  'Oración Completa ${e.key + 1}',
                                ),
                              ),
                              const SizedBox(height: 8),
                              RichTextField(
                                ...
                                label: 'Palabra (Gap)',
                                decoration: _inputDeco('Palabra (Gap)'),
                              ),
                            ],
                          ),
                        ),
                      ...
                      TextField(
                        ...
                        keyboardType: TextInputType.number,
                        decoration: _inputDeco('Máximo de palabras'),
                      ),
                    ] else if (_type == 'record_audio') ...[
                      Center(
                        child: Text(
                          'El estudiante grabará audio (hasta 45s).',
                          style: TextStyle(
